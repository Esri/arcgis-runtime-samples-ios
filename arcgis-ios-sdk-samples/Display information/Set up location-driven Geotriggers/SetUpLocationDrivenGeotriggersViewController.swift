...
class SetUpLocationDrivenGeotriggersViewController: UIViewController, AGSPopupsViewControllerDelegate {
    ...
    func setupMapView() {
        ...
        map.load { [weak self] _ in
            ...
            let locationDataSource = self.makeDataSource(polylineJSONString: self.walkingTourPolyineJSON)
    ...
    @IBAction func pointOfInterestButtonTapped(_ sender: UIBarButtonItem) {
        if !nearbyPOINames.isEmpty {
            ...
            presentAlert(title: "No Nearby Point-of-interest", message: "There are no nearby places to checkout.")
        }
    }
